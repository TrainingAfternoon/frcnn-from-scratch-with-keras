...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
    ...
    for jk in range(R.shape[0]//num_rois + 1):
        ...
        for ii in range(P_cls.shape[1]):
            print('\tcertainty =', P_cls[0,ii,:])

            if np.max(P_cls[0,ii,:]) < 0.0 or np.argmax(P_cls[0,ii,:]) == (P_cls.shape[2] - 1):
                continue

            cls_name = "stamp"#class_mapping[np.argmax(P_cls[0,ii,:])]
            ...
            probs[cls_name].append(P_cls[0,ii,0])
    ...
    for key in bboxes:
        ...
        for jk in range(new_boxes.shape[0]):
            ...
            with open("test-frcnn-out.txt","a") as datafile:
                datafile.write(f'{img_name},{real_x1},{real_y1},{real_x2},{real_y2},{key},{_id},{int(100*new_probs[jk])}\n')

            textLabel = '({}) {}: {}'.format(_id, key,int(100*new_probs[jk]))
            _id += 1
            ...
            cv2.rectangle(img, (textOrg[0] - 5,textOrg[1]+baseLine - 5), (textOrg[0]+retval[0] + 5, textOrg[1]-retval[1] - 5), (0,0,255), -1)
